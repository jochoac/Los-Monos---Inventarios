...
namespace Logica
{
    public class Reportes
    {
        ...
        public string PDF_venta(DataGridView Ta, string Nombreloc, string fecha, long codigo)
        {
            ...
            string filename = "C:\\ProgramData\\Factura" + codigo + ".pdf";
        ...
        public DataGridView retorno(DataGridView v)
        {
            for (int i = 0; i < v.RowCount; i++)
            {
                ...
                for (int j = i + 1; j < v.RowCount; j++)
                {
                    ...
                    {
                        int cant = int.Parse(v.Rows[i].Cells[3].Value.ToString());
                        cant += int.Parse(v.Rows[j].Cells[3].Value.ToString());
                        v.Rows[i].Cells[3].Value = cant;
        ...
        public DataTable Consolidado(string local, string fecha)
        {
            SqlConnection a = con.conectar();
            a.Open();
        ...
        public DataTable Consulta_historial()
        {
            SqlConnection a = con.conectar();
            a.Open();
            SqlCommand consultar = new SqlCommand("Select * from Historial order by codigo_historia desc", a);
            SqlDataReader Consu = consultar.ExecuteReader();
            System.Data.DataTable Tabla = new System.Data.DataTable();
            Tabla.Load(Consu);
            a.Close();
